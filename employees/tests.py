...
class EmployeeListTestCase(APITestCase):
    ...
    def test_get_employee_detail(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(reverse("employees-detail", kwargs={"pk": self.employee.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_search_by_nickname(self):
        ...
        self.assertTrue("roger" in response.json()[0].get("nickname"))
    ...
    def test_search_by_firstname(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(bool(response.json()))
        self.assertTrue("foo" in response.json()[0]["user"].get("first_name").lower())
    ...
    def test_search_by_lastname(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(bool(response.json()))
        self.assertTrue("bar" in response.json()[0]["user"].get("last_name").lower())

    def test_search_by_email(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(reverse("employees-list"), {"search": "foobar"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(bool(response.json()))
        self.assertTrue("foobar" in response.json()[0]["user"].get("email"))
